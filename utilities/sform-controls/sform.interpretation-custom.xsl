...
<xsl:stylesheet
	...
	<xsl:template name="sform:formi">
		...
		<xsl:if test="$entry-data != '' or $entry-data/*">
			<xsl:call-template name="sform:interpretation-custom.entry">
			...
			<xsl:call-template name="sform:interpretation-custom.filters">
				<xsl:with-param name="entry-data" select="$entry-data"/>
			...
			<xsl:call-template name="sform:interpretation-custom.fields">
	...
	<!--
		Name: sform:interpretation-custom.entry
		Description: Interprets the results for entry.
		Returns: XML
	-->
	<xsl:template name="sform:interpretation-custom.entry">
		...
		<xsl:variable name="default_selection">
			<xsl:call-template name="sform:validation-tpl-sel">
				<xsl:with-param name="handle" select="'entry'"/>
		...
		<xsl:variable name="result-nodeset">
			<xsl:for-each select="$v_selection">
				...
				<xsl:variable name="item_status">
					<xsl:choose>
						<xsl:when test="$entry-data/entry/@result = 'success'">
							<xsl:value-of select="$sform:STATUS_SUCCESS"/>
						</xsl:when>
						<xsl:when test="$entry-data/entry/@result = 'error'">
				...
				<xsl:if test="status = $sform:STATUS_ANY or status = $item_status">
					...
					<xsl:variable name="item_original">
						<entry>
							<xsl:for-each select="$entry-data/entry/@*">
								<xsl:copy/>
							</xsl:for-each>
						</entry>
					</xsl:variable>
					...
					<xsl:variable name="item_message">
						<xsl:choose>
							...
							<!-- Symphony -->
							<xsl:when test="$entry-data/entry/message[ not(@status) ]">
								<xsl:copy-of select="$entry-data/entry/message[ not(@status) ]/* | $entry-data/entry/message[ not(@status) ]/text()"/>
							</xsl:when>
	...
	<!--
		Name: sform:interpretation-custom.filters
		Description: Interprets the results for filters.
		Returns: XML
	-->
	<xsl:template name="sform:interpretation-custom.filters">
		<xsl:param name="entry-data"/>
		<xsl:param name="selection"/>
		<xsl:param name="messages"/>
		<xsl:param name="defaults"/>
		...
		<xsl:variable name="default_selection">
			<xsl:for-each select="$entry-data/filters/filter">
				<xsl:call-template name="sform:validation-tpl-sel">
		...
		<xsl:variable name="result-nodeset">
			<xsl:for-each select="$v_selection">
				...
				<!-- Original information -->
				<xsl:variable name="item_original" select="$entry-data/filters/filter[ @name = current()/@handle ]"/>
	...
	<!--
		Name: sform:interpretation-custom.fields
		Description: Interprets the results for fields.
		Returns: XML
	-->
	<xsl:template name="sform:interpretation-custom.fields">
		...
		<xsl:variable name="default_selection">
			<xsl:for-each select="$entry-data/fields/*">
				<xsl:call-template name="sform:validation-tpl-sel">
					<xsl:with-param name="handle" select="name(.)"/>
					<xsl:with-param name="status" select="$sform:STATUS_ERROR"/>
				</xsl:call-template>
		...
		<xsl:variable name="result-nodeset">
			<xsl:for-each select="$v_selection">
				...
				<!-- Original information -->
				<xsl:variable name="item_original" select="$entry-data/fields/*[ name() = current()/@handle ]"/>
